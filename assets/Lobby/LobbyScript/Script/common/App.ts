                    ...
                    Http.get(App.API_IBC, { t: "Login", nn: Configs.Login.Nickname, at: Configs.Login.AccessToken }, (err, res) => {
                        ...
                        if (res["code"] == 0) {
                            if (Configs.App.IS_PRO == true && Configs.Login.UserType != "2") {
                                var url = "https://cb5chau.com/deposit_processlogin.aspx?lang=vn&token=" + res["data"]["data"];
                                if (cc.sys.isMobile == true) {
                                    url = "https://cb5chau.com/deposit_processlogin.aspx?lang=vn&token=" + res["data"]["data"];
                                }
                                cc.sys.openURL(url);

                            }
                            else {
                                var url = "http://cb5chau.com/deposit_processlogin.aspx?lang=vn&token=" + res["data"]["data"];
                                if (cc.sys.isMobile == true) {
                                    url = "http://cb5chau.com/deposit_processlogin.aspx?lang=vn&token=" + res["data"]["data"];
                                }
                                cc.sys.openURL(url);
                            }
                        }
                ...
                if (res["data"] == null) {
                    ...
                    if (balance < 10000 && isPlayNow == false) {
                        ...
                        Http.get(App.API_EBET, { t: "Login", nn: Configs.Login.Nickname, at: Configs.Login.AccessToken }, (err, res) => {
                            App.instance.showLoading(false);
                            if (res["res"] == 0) {
                                let url = "https://cb5chau.com/h5/72895c?username=%s&accessToken=%s";
