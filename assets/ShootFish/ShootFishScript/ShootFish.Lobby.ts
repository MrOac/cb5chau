...
export default class Lobby extends cc.Component {
	...
	@property(cc.Label)
    lblslotz: cc.Label = null;
	@property(cc.Label)
    lblslotz100: cc.Label = null;
	@property(cc.Label)
    lblslotz1k: cc.Label = null;
    ...
    onLoad() {
        ...
        ShootFishNetworkClient.getInstance().checkConnect((isLogined) => {
            ...
            if (Configs.Login.CoinFish <= 0) {
                App.instance.confirmDialog.show3("Tiền trong Bắn Máy Bay của bạn đã hết, bạn có muốn chuyển tiền vào không?", "Có", (isConfirm) => {
                    if (isConfirm) {
                ...
                switch (route) {
					...
					cc.log("route: " + route + " data: "+ JSON.stringify(data));
                       // this.tabsListGame.updateItemJackpots("shootfish", data["14"], false, data["24"], false, data["34"], false);
                        // this.buttonListJackpot.updateFish(data["14"], data["24"], data["34"]);
                        // this.topHu.updateFish(data["14"], data["24"], data["34"]);
                        this.FISH10000 = data["34"];
						this.FISH1000 = data["24"];
						this.FISH100 = data["14"];
						this.lblslotz.string = Utils.formatNumber(this.FISH10000);
						this.lblslotz100.string = Utils.formatNumber(this.FISH100);
						this.lblslotz1k.string = Utils.formatNumber(this.FISH1000);
                        break;
                }
            }, this);
    }
